...
export default function usePeriodicTokenRefresh({
    ...
    onFailure: (reason: InvalidTokenReason | undefined, from: RPCError | unknown) => void
}) {
    ...
    const onFailureWithLogging: typeof onFailure = (reason, from) => {
        if (reason) {
            ColorDebug.browser().error("Failed to refresh token due to known error: " + InvalidTokenReason[reason], {
                textColor: "red"
            })
        } else {
            ColorDebug.browser().error("Failed to refresh token due to unknown error: \n" + JSON.stringify(
                formatError(from), null, 2
            ), {
                textColor: "red"
            })
        }
        ...
        try {
            ...
            if (RPCError.isLike(e)) {
                ...
                if (typeof rE.cause === "string") {
                    if ((Object.values(InvalidTokenReason) as string[]).includes(rE.cause)) {
