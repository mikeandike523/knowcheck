...
function updateQuestion(criteriaName) {
  document.getElementById("question").innerText =
    "What are the INPATIENT criteria for: " + criteriaName + "?";
...
async function submitAnswer() {
  ...
  document.getElementById("answerDetails").innerHTML =
    "<strong>WAIT FOR IT...</strong>"; // Show waiting message
...
async function analyzeAnswer(userAnswer) {
  try {
    const result = await firebaseFetch("analyzeAnswer", {
      answer: userAnswer,
      questionNumber: currentDiagnosisNumber,
    });
    return result;
...
function sendEmail() {
  ...
  const body = encodeURIComponent(
    "I got " +
      currentScore +
      ' points - and the secret code is "UNIMAGINATIVE SECRET CODE"',
  );
